...
void CopyMask(int from, int to, int MS) 
 { int i; for (int i= 0; i< MS; i++) MASKQUE[to][i] = MASKQUE[from][i]; MKEY[to] = MKEY[from];
 return;
 }
double PURGEQ() { double SumQ= 0; for (int i= MQXFront; i <= MQXRear; i++) SumQ += MKEY[i]; return SumQ;}
void QDUMP(int MS) { char OK;
int i=MQXFront,j= MQXRear, QS = j-i+1;; printf( " QS = %d\n", QS);
for (int x = i; x <= j; x++)  { printf (" MASKQUE[%d] %p : ", x, MASKQUE[x]); 
for (int k= 0; k < MS; k++ ) printf( " %x ", MASKQUE[x][k]) ; printf( " MKEY = %lf \n", MKEY[x]);
} 
//cin >> OK;
}
...
bool CHECKMULT(float Prob[], int MaskSize, int IX) { double XM = 1.0;for ( int i=0; i< MaskSize; i++) {if (MASKQUE[IX][i]== 1) XM *= Prob[i]; if (MASKQUE[IX][i]== 0) XM *= (1-Prob[i]);
                    } return (XM == MKEY[IX] );
                    }
int Seq_No(int i, int j , N_Type Nodes[], E_Type Edges[]) { 
 ...
 bool Opermask(int N,unsigned char *NewMask,N_Type Nodes[],E_Type Edges[], int src, int t) {
	  ...
	  while ( rear >= front) {
	   ...
	   for (int dx=0; dx < d; dx++) { int j = Edges[e_l+dx].dst; int sn = Edges[e_l+dx].SeqNo; //printf("0%xHERE %d %d %d|", V, j, sn);
	      if (NewMask[sn] == 1 || NewMask[sn] == 0xFF ) if(!Visited[j]) {Visited[j] = true;rear++; Queue[rear] = j; Parent[j] = V;}
...
{
	 int M = G->M, N = G->N; unsigned char GT = G->GT; int sn; 
	 clock_t start= clock(), end; int mTime;	 
	 bool OP = false;
	 double MULT, NewMULT; 
	 int e_l; int d, dx; short V, j; ;
	 int MaskSize;
	 for(int i =0; i < MAX_Q_SIZE; i++) MKEY[i]= 1.0;
	 if (GT == 'U') MaskSize = M/2; else MaskSize = M; 
	 
	 if (MASKQUE) printf ("OK %p \n", MASKQUE); else printf("Memory Problem!!");
	 for (int i=0; i< MAX_Q_SIZE; i++) { MASKQUE[i] = new unsigned char[MaskSize]; if (!MASKQUE[i]) {printf("NULL POINTER"); break;} }
	 ...
	 printf(" MaskSize = %d MAX_Q_SIZE = %d\n",  MaskSize, MAX_Q_SIZE);
 	 ...
 	 short *TQ = new short[MaskSize]; int TQTOP; // STACK TO store sn for unmarked edges ON AN s-t-path -> Rel_08.cu
 	 
 	 bool *Visited= new bool[N]; short *Parent= new short[N];  short *Queue = new short[N]; // BFS src -> t
 	  
 	 /*________________START PROCESSING MASK QUEUE ________________________________*/
 	 
 	 MQXRear++; for (int i = 0; i < MaskSize; i++) MASKQUE[MQXRear][i] =  Mask[i]; MKEY[MQXRear] = 1.0; 
 	 Q_SIZE = 1;//for (int i = 0; i < MaskSize; i++) printf("0x%.2x  ",MASKQUE[0][i]); printf("\n");
 	 int T = 0;
 	 
 	 while (Q_SIZE > 0) { //printf("BEFORE:: ");QDUMP(MaskSize);
 check:   //for (int i =0; i < 10; i++) printf(" %p : ",MASKQUE[i]); printf("SIZE = %d \n", Q_SIZE);
 	  //printf ("MQXFront = %d T = %d ", MQXFront, T);
 	  for( int i=0; i < MaskSize;i++) Mask[i] = MASKQUE[MQXFront][i]; MULT = MKEY[MQXFront];   //if (!CHECKMULT(Prob, MaskSize, MQXFront)) printf("ERROR1: IX = %d T= %d \n", MQXFront, T);
 	  if (Q_SIZE > 1) {CopyMask(MQXRear, MQXFront, MaskSize); MQXRear--;} 
 	  Q_SIZE -= 1; if (Q_SIZE == 0) MQXRear = -1;
 	  ...
 	  //printf("AFTER::  ");QDUMP(MaskSize);
 	   ...
 	   for (int i=0; i <N; i++) Visited[i]= false; OP = false; 
	   for (int i=0; i <N; i++) Parent[i]= -1;
	  ...
	  while ( rear >= front) {
	   V = Queue[front]; front++;       //////// //printf("V = %d|", V);
           d=Nodes[V].degree;   
           e_l = Nodes[V].Offset; 
	   for (dx=0; dx < d; dx++) { j = Edges[e_l+dx].dst; sn = Edges[e_l+dx].SeqNo; //printf("HERE %d %d %d|", V, j, sn);
	      if (Mask[sn] == 1 || Mask[sn] == 0xFF ) 
	        if(!Visited[j]) {Visited[j] = true;rear++; Queue[rear] = j; Parent[j] = V;}
	 ...
	 if (OP){ 
	         // Climb up the path from t to s; Store sn's for unmarked edges
	          int i = Parent[t]; int j = t; int sn; TQTOP= -1; 
	          while ( j != src) {
	           sn = Seq_No(i,j, Nodes, Edges); 
	           if( Mask[sn] == 0xFF)  { TQTOP++; TQ[TQTOP] = sn; //cout << "(" << i << j << sn << ")" ;
	           }
	           j = i; i = Parent[j];
	           }
	           ...
	           while (TQTOP >= 0) { sn = TQ[TQTOP]; TQTOP--;  //Create EXTRA MASKS from TQ
	             ...
	             NewMask[sn] = 0; NewMULT = NewMULT * (1 - Prob[sn]); //printf(" Q SIZE = %d\n", Q_SIZE);
	             if(Opermask(N,NewMask,Nodes,Edges, src, t)) { // DOES NOT HAVE A CUT
	               if (Q_SIZE < MAX_Q_SIZE) { //printf("HEER %d\n", MQXRear);
	                  MQXRear++;  Q_SIZE++;
	                  for ( int i=0; i< MaskSize; i++) MASKQUE[MQXRear][i] = NewMask[i];  MKEY[MQXRear] = NewMULT;
	                  //printf("BEFORE-:: ");QDUMP(MaskSize);
	     ...
	     // Add Increment to rel[tx],
	     	NP++;   if(isnan(MULT)) printf("ISNAN\n");
	     	if (!isnan(MULT)) {rel += MULT; //printf("NP = %d MULT  %.20lf rel %.20lf +++\n", NP, MULT, rel);
	     	}
	     	mTime = (int) (((float) clock() - start)/  CLOCKS_PER_SEC) ; 
	     	// if (mTime > 120) {Eps += PURGEQ();break; 	}
	     	}   // if(OP)                           
 	  end = clock(); mTime = (int) (((float) end - start)/  CLOCKS_PER_SEC) ;	
...
int main(){
	...
	cout << "READ OK\n";
	Visited= new bool[N]; if(!Visited) printf ("OUT OF MEMORY");
        Parent= new short[N]; if(!Parent) printf ("OUT OF MEMORY");
        Queue = new short[N]; if(!Queue) printf ("OUT OF MEMORY");
	N_Type *Nodes = new N_Type[N]; E_Type *Edges = new E_Type[M]; if(!Edges) printf ("OUT OF MEMORY");Nodes[0].Offset = 0;	
	int E_X = 0;
	for (int n=0; n < N; n++) {
	 ...
	 Prob = new float[NumOfEdges]; if(!Prob) printf ("OUT OF MEMORY");
	   
	 for (int i =0; i < M; i++) {int sn = Edges[i].SeqNo; Prob[sn]= Edges[i].prob;
