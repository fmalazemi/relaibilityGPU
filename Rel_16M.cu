...
__device__ void CopyMask(MASKTYPE MASKQUE[], double MKEY[], int from, int to, int MS) 
 { for (int i= 0; i< MS; i++) MASKQUE[to][i] = MASKQUE[from][i]; MKEY[to] = MKEY[from];
 return;
 }
__device__ double PURGEQ(int MQXFront, int MQXRear, double MKEY[]) { 
     double SumQ= 0; for (int i= MQXFront; i <= MQXRear; i++) SumQ += MKEY[i]; return SumQ;
}
...
__device__ void TID_TO_MASK(unsigned char * mask, int tid, int MaskSize , int TID_SIZE) {
                      ...
                      if (TID_SIZE <= MaskSize) for (int e = 0; e < TID_SIZE; e++) { int m = 1 << e; if ( tid & m) mask[e] = (unsigned char) 0x1; else mask[e] = 0x0;}
                      else  for (int e = 0; e < MaskSize; e++) {int m = 1 << e; if ( tid & m) mask[e] = (unsigned char) 0x1; else mask[e] = 0x0;}
    } 
...
__global__  void TC( Graph_Type *G, N_Type Nodes[], E_Type Edges[], float Prob[], int src, int t, double* d_Brel, double *d_Epsilon, int* d_NP, int TID_SIZE, int Device_ID)
{
	...
	__shared__ short Queue[NTH][MaxN];   
	clock_t start= clock(); int mTime;
	...
	if (GT == 'U') MaskSize = M/2; else MaskSize = M; 
	 ...
	 __syncthreads();
	 if (TID == 0) if (MASKQUE) printf ("OKKKKKKKK %p \n", MASKQUE);
 	 if (MaskSize <= 62) if (TID >= ((long int) 2 << (MaskSize-1 ))) { //printf("OOPS"); //??????????????????????
	       return;}	
 	 ...
 	 MQXRear++; for (int i = 0; i < MaskSize; i++) MASKQUE[MQXRear][i] =  Mask[i]; MKEY[MQXRear] = MULT; int Q_SIZE = 1;
 	 //int T = 0;
 	 while (Q_SIZE > 0) { 
 	  for( int i=0; i < MaskSize;i++) Mask[i] = MASKQUE[MQXFront][i]; MULT = MKEY[MQXFront];   
 	  //printf ("MASK READ FROM Q :");for (int i = 0; i < MaskSize; i++) printf("0x%.2x  ",Mask[i]); printf("\n");
 	  if (Q_SIZE > 1) {CopyMask(MASKQUE, MKEY, MQXRear, MQXFront, MaskSize); MQXRear--;} 
 	  ...
 	  for (int i=0; i <N; i++) Visited[tx][i]= false; OP = false; int front=0, rear= -1;
 	  for (int i=0; i <N; i++) Parent[tx][i]= -1;
	  
	  rear++; Queue[tx][rear] = (short) src; 
	  while ( rear >= front) {
	   V = Queue[tx][front]; front++;  //printf("V = %d|", V);
           d=Nodes[V].degree;   
           e_l = Nodes[V].Offset; 
	   for (dx=0; dx < d; dx++) { j = Edges[e_l+dx].dst; sn = Edges[e_l+dx].SeqNo; //printf("HERE %d %d %d|", V, j, sn);
	      if (Mask[sn] == 1 || Mask[sn] == 0xFF ) 
	         if(!Visited[tx][j]) {Visited[tx][j] = true;rear++; Queue[tx][rear] = j; Parent[tx][j]=V;}
	      if (Visited[tx][t]) {OP =true; break;}                
	      }
	     if (Visited[tx][t]) {OP =true; break;} //if (TID == 13) for (int i = 0; i<N; i++) printf("%d /",Queue[i]); printf("front %d rear %d ///", front, rear);
	   }  // While
