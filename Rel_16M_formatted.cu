...
__device__ void TID_TO_MASK(unsigned char * mask, int tid, int MaskSize , int TID_SIZE) {
	for (int e = 0; e < MaskSize; e++) 
		mask[e] = (unsigned char) 0xFF; // UNMARKED EDGE
	if (TID_SIZE <= MaskSize) 
		for (int e = 0; e < TID_SIZE; e++) { 
			int m = 1 << e; 
			if ( tid & m) 
				mask[e] = (unsigned char) 0x1; 
			else
				mask[e] = 0x0;
		}
	else
...
__global__  void TC( Graph_Type *G, N_Type Nodes[], E_Type Edges[], float Prob[], int src, int t, double* d_Brel, double *d_Epsilon, int* d_NP, int TID_SIZE, int Device_ID)
{
	...
	NP[tx] = 0; 
	 ...
	 if (TID == 0) 
		if (MASKQUE) 
			printf ("OKKKKKKKK %p \n", MASKQUE);
 	 if (MaskSize <= 62)
		if(TID >= ((long int) 2 << (MaskSize-1 ))) { 
			//printf("OOPS"); //??????????????????????
	       return;
		}
 	 if (TID == 0)
		printf("TID SIZE = %d MaskSize = %d MAX_Q_SIZE = %d\n", TID_SIZE, MaskSize, MAX_Q_SIZE);
	...
	for (int e = 0; e < MaskSize; e++) 
		if (Mask[e] == 1 ) 
			...
			MULT *= (1- (double)Prob[e]);
 	 
 	 MQXRear++; 
	for (int i = 0; i < MaskSize; i++) 
		MASKQUE[MQXRear][i] =  Mask[i]; 
	MKEY[MQXRear] = MULT; 
	int Q_SIZE = 1;
 	 //int T = 0;
 	 while (Q_SIZE > 0) { 
		for( int i=0; i < MaskSize;i++) 
			Mask[i] = MASKQUE[MQXFront][i]; 
		MULT = MKEY[MQXFront];   
