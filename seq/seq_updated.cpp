...
// Constants
//=============================================================================
#define MAX_Q_SIZE 1300000    // Maximum queue size
...
//===================================================================================
inline bool findPath(ReliabilityContext* ctx, bool flag) {
    int N = ctx->N;
    int src = ctx->src;
    int t = ctx->t;
    N_Type* Nodes = ctx->Nodes;
    ...
    while (front < rear) {
        ...
        for (int dx = 0; dx < d; dx++) {
            ...
            if (!Visited[j] && (Mask[sn] == MASK_UP || (flag == true && Mask[sn] == MASK_NOMARK))) {
                Visited[j] = true;
                Parent[j] = V;
                Queue[rear++] = j;
...
void computeReliability(ReliabilityContext* ctx) {
    ...
    while (!ctx->pq.empty()) {
        ...
        while (ctx->PathStackTop >= 0) {
            ...
            //memcpy(ctx->NewMask, ctx->Mask, ctx->MaskSize);
            ctx->NewMask[sn] = MASK_UP;
            logNewMult = logMult + ctx->LogProb[sn];
