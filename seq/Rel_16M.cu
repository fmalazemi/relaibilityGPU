...
int main() {
  ...
  for (int i = 0; i < NGpus; i++) {
    ...
    cudaMalloc( & d_Graph[i], sizeof(Graph_Type)  );
    ...
    cudaMemcpyAsync(d_Graph[i], G1, sizeof(Graph_Type) * NBL, cudaMemcpyHostToDevice);
    ...
    cudaMemset(d_Brel[i], 0.0, sizeof(double) * NBL);
    cudaMemset(d_BEps[i], 0.0, sizeof(double) * NBL);
  } // End Alloc & CPY
  ...
  double h_rel = 0.0;
  double rel[NGpus] = { 0.0};
  double h_Eps = 0.0;
  double Eps[NGpus] = { 0.0 };
  int h_NP = 0;
  int NP[NGpus] = {0};
  cout << "BACK ";
